...
///
/// `TSS_KEY` is generated by TssCodeGen (`TSS.Rust/src/tpm_types.rs`) as
/// `#[derive(Debug, Clone, Derivative)]` over a plain `Vec<u8> privatePart`, and generated files
/// must not be hand-edited. `Drop` may be implemented anywhere in the defining crate, so the
/// wipe can be added from here; `Debug` cannot, because the generated `derive` already provides
/// one and a second implementation would not compile.
///
/// The consequence is worth stating plainly rather than leaving implied. [`RsaKeyParts`] both
/// redacts its `Debug` and zeroizes on drop, but it holds the prime only for the few microseconds
/// of [`TSS_KEY::create_key`]; the copy that lives for the rest of the process is this one. After
/// this implementation that copy is wiped when it is dropped — every clone of it too, exactly as
/// for `RsaKeyParts` — but it is **still rendered in full by `{:?}`**, so a log line or an error
/// path that formats a `TSS_KEY` prints the private RSA factor. Closing that requires the
/// generator to emit a redacting `Debug` (or a `Zeroizing`/newtype field) for the types that
/// carry secrets: `TSS_KEY::privatePart` here, and `TPM_HANDLE::auth_value`, which has the same
/// problem and is far too widely constructed and cloned for a hand-written `Drop` in this file to
/// be worth the blast radius. That is a `TssCodeGen/src/CGenRust.cs` change and follow-up work.
///
...
impl TSS_KEY {
    ...
    /// which is wiped on drop by the [`Drop`] implementation above. Note its limits: `TSS_KEY`
    /// derives `Debug`, so the prime is still printable.
